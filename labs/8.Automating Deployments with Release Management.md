...
8.  Edit the **Tomcat** task parameters to match the
    ...
    Make sure you still use the old password of the vmadmin account if you have changed the password of the vmadmin account after login. The Tomcat server uses a dedicated vmadmin account in the tomcat-users xml file. Passing the new password will result in a curl error: the requested URL returned error: 401.    
